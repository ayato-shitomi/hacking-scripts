...
def	from_base64_exif(path):
	print("=== plain text and base64 decode on exif command ===")
	with open(path) as fi:
		n = 0
		for i in fi:
			tmp = i.split(": ")[1].replace("\n", "")
			ch_flag(path + ":" + str(n), tmp, 0)
			try:
				ch_flag(path + ":" + str(n) + " '" + tmp + "' -> ", base64.b64decode(tmp), 1)
			except:
				pass
			n = n + 1
	print()

def from_base64_string(path):
	...
	with open(path) as fi:
		for i in fi:
			...
			ch_flag(path + ":" + str(n), tmp, 0)
			try:
				ch_flag(path + ":" + str(n) + " '" + tmp + "' -> ", base64.b64decode(tmp), 0)
			except:
				pass
			n = n + 1
	print()

from_base64_string("./2023-03-04_23-06-01-592408_cat-jpg/strings.txt")
from_base64_exif("./2023-03-04_23-06-01-592408_cat-jpg/exiftool.txt")
